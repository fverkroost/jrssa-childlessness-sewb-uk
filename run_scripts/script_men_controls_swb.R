run_my_model = function(
  ...
  for (i in 1:length(datalist)){
    datalist[[i]]$lifeSatisfaction = as.integer(datalist[[i]]$lifeSatisfaction)
    datalist[[i]]$logCorrectedMonthlyGrossPay = as.numeric(datalist[[i]]$logCorrectedMonthlyGrossPay)
    datalist[[i]]$eventualTypeChildless = as.factor(datalist[[i]]$eventualTypeChildless)
    datalist[[i]]$numberChildrenNaturalCategorical = factor(datalist[[i]]$numberChildrenNaturalCategorical)
    datalist[[i]]$childArrivedSinceLastWave = factor(datalist[[i]]$childArrivedSinceLastWave)
    datalist[[i]]$maritalStatus_combined = factor(datalist[[i]]$maritalStatus_combined)
    datalist[[i]]$cohabitation = factor(datalist[[i]]$cohabitation)
    datalist[[i]]$education_binary_bwimp = factor(datalist[[i]]$education_binary_bwimp)
    datalist[[i]]$economicActivity = factor(datalist[[i]]$economicActivity)
    datalist[[i]]$healthStatus_binary = factor(datalist[[i]]$healthStatus_binary)
    datalist[[i]]$ID = as.integer(datalist[[i]]$ID)
    datalist[[i]]$poly_time_1 = as.numeric(datalist[[i]]$poly_time_1)
    datalist[[i]]$poly_time_2 = as.numeric(datalist[[i]]$poly_time_2)
    datalist[[i]]$poly_time_3 = as.numeric(datalist[[i]]$poly_time_3)
  }
...
run_my_model(
  ...
  sub_folder = "controls_test_factor_child_arrival",
  dep_vars = "lifeSatisfaction",
  other_dep = "logCorrectedMonthlyGrossPay",
  number_imputations = 4,
  gender = "men",
  control_vars = c("numberChildrenNaturalCategorical", "childArrivedSinceLastWave", "logCorrectedMonthlyGrossPay",
                   "maritalStatus_combined", "cohabitation", "education_binary_bwimp", "economicActivity", "healthStatus_binary"),
  priors = "informative",
  poly_order_fe = 3,
  poly_order_re = 1,
