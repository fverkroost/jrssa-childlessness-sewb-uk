run_my_model = function(
  ...
  for (i in 1:length(numbers)){
    ...
    datalist[[i]][, indep_vars] = as.character(datalist[[i]][, indep_vars])
    datalist[[i]][, indep_vars] = ifelse(datalist[[i]][, indep_vars] %in% c("Divorced/separated", "Widowed"), "Divorced/separated/widowed", datalist[[i]][, indep_vars])
...
run_my_model(
  ...
  indep_vars = 'maritalStatus_combined', 
